...
In BambooHR, only managers/admins can create API keys. Everyone else has no way to run `init.sh` or submit via the API. **Offline mode** solves this: a manager exports a shared config once, and teammates use the skill to build their hours table conversationally, then enter it into BambooHR by hand.

The skill auto-detects offline mode when no `BAMBOOHR_API_KEY` is set. You can also force it with `/timesheet offline`.

### For the manager (one-time): export a shareable config

With your API key configured, generate a config that contains **only projects and tasks** — your personal `employeeId` is stripped out:

```bash
.claude/skills/timesheet/scripts/init.sh --export
# → writes config.shared.json (projects/tasks only)
#   (use --export /path/to/file.json for a custom output path)
```

If you already have a local `config.json`, `--export` just strips the `employeeId` from it (no API call). Otherwise it fetches from the API first.

Share `config.shared.json` with your team **privately** (Slack, Drive, etc.). Don't commit it — it's company-internal data and is gitignored by default.

### For teammates: use it without an API key

1. Save the file your manager shared as `.claude/skills/timesheet/config.json`.
2. Make sure no `BAMBOOHR_API_KEY` is set (you don't need one).
3. Use the skill:

```
/timesheet                 # describe your day, review the table
/timesheet offline         # force offline mode explicitly
/timesheet calendar        # import meetings from Google Calendar (works offline)
```

The skill builds and refines your entries table, then outputs a final table for you to **copy into BambooHR → Time Tracking manually**. Nothing is submitted via API.

### What works offline

| Feature | Offline |
|---------|---------|
| Conversational entry building | ✅ Yes |
| Final table for manual entry | ✅ Yes (no submission) |
| Google Calendar import (`calendar`) | ✅ Yes (uses MCP, not the BambooHR API) |
| Show entries (`show`) | ❌ Needs the API |
| Team review (`review`) | ❌ Needs the API |
| Direct API submission | ❌ Enter hours manually instead |
...
├── README.md             # This file
├── config.json           # Projects/tasks for this user (not committed to git)
├── config.shared.json    # Shareable config from init.sh --export, no employeeId (not committed)
├── review-config.json    # Team review rules, created on first review (not committed to git)
└── scripts/
    ├── init.sh           # Bootstrap/detect: discover projects/tasks; --check (mode), --export (shareable config)
    ├── submit.sh         # Submit entries or show existing ones (online)
...
| GET | `/api/gateway.php/{domain}/v1/employees/directory` | Find direct reports (team review) |
| GET | `/api/v1/time_off/requests?employeeId=X&start=Y&end=Z&status=approved` | Approved time off (team review) |
...
| No API key (non-manager) | Use offline mode — get the shared `config.json` from your manager and save it to the skill directory. See [Offline Mode](#offline-mode-no-api-key). |
